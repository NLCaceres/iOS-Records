...
final class RepositoryGenericsTests: XCTestCase {
    func testOnSuccess() async throws {
        let someEntity = try? await getResult { return .success(1) }
    ...
    func testOnFailure() async throws {
        ...
        let someResult = try? await getResult(getData: someThrowingClosure)
        XCTAssertNil(someResult) // Error should have been thrown so nil gets returned by getResult
    }
}
